...
const BetButton = ({ onBet, disabled, isWalletConnected }: BetButtonProps) => {
  ...
  const handleBet = async () => {
    ...
    try {
      ...
      toast.loading(`Sending ${betAmount} SOL transaction to Solana blockchain...`);
      
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      onBet(betAmount);
      
      toast.success(`Transaction confirmed! Your bet of ${betAmount} SOL has been placed.`);
    } catch (error) {
